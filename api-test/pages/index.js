...
export default function Home() {
	...
	const fetchProjectDependencies = async (
		...
		const url = packageVersion
			? `https://libraries.io/api/${platform}/${name}/${version}/dependencies?api_key=${apiKey}`
			: `https://libraries.io/api/${platform}/${name}?api_key=${apiKey}`;
	...
	return (
		<div style={{ textAlign: "center" }}>
			...
			<section>
					...
					<div>
						<div>
							...
							<input
								...
								required
								onChange={(event) =>
									setPackegeName(event.target.value)
								}
							/>
						</div>
						<div>
				...
				{!loading && packageInfo && (
					<ProjectDependencies packageInfo={packageInfo} />
				)}
